...
println("---| masking galactic foregrounds and point sources")
...
CoordRand = np.load("$path$coordname"".npy")
@time StackImage = StackRandImg(CoordRand);
np.save("$path""stackedimg_$coordname",StackImage)
